...
3. **Rank** — `events list` sends your upcoming events and preference profile to Claude, which returns them scored 1–10 with a one-line note explaining the match. The model is `claude-opus-4-8` by default; override it with the `ANTHROPIC_MODEL` environment variable.
